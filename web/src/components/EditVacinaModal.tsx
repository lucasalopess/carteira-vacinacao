...
interface EditVacinaModalProps {
    isOpen: boolean;
    vacina: Vacina | null;
    onClose: () => void;
...
export function EditVacinaModal({isOpen, vacina, onClose, onSubmit, onDelete}: EditVacinaModalProps) {
    ...
    useEffect(() => {
        if (isOpen && vacina) {
            setNome(vacina.nome);
            setIdadeInicial(String(vacina.idadeInicial));
            setIntervaloDoses(String(vacina.intervaloDoses));
            setRecorrente(vacina.recorrente || false);
            setQtdDoses(vacina.qtdDoses != null ? String(vacina.qtdDoses) : '');
            setDosesReforco(vacina.dosesReforco);
            setQtdReforco(vacina.qtdReforco != null ? String(vacina.qtdReforco) : '0');
        }
    }, [isOpen, vacina]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            showError(err instanceof Error ? err.message : 'Erro ao atualizar vacina');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleDelete = async () => {
        ...
        try {
            setIsLoading(true);
            await onDelete(vacina.id);
            setShowDeleteConfirm(false);
            onClose();
        } catch (err) {
            showError(err instanceof Error ? err.message : 'Erro ao excluir vacina');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <>
            ...
            <ConfirmModal
                ...
                message={`Tem certeza que deseja excluir a vacina "${vacina?.nome}"? Esta ação não pode ser desfeita.`}
